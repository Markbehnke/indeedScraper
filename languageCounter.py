...
languages = ["python", "c++", "sql", "html", "php", "javascript", "matlab", "c#", "swift", 'react', 'rust', 'css']
    ...
    with open("./logs\\" + individualFiles, 'r', encoding="utf-8") as file:
        ...
        for entry in data.split('scrapedjobid'):
            ...
            if re.search(r'\br\b', entry):
                wordCounts["R"]+=1
            if re.search(r'\bgo\b', entry):
                wordCounts["Go"]+=1
            if re.search(r'\bjava\b', entry):
                wordCounts["Java"]+=1
