...
def stemming(desc, oldFile):
    ...
    with open(f"./processedLogs/{oldFile}", 'a', encoding="utf-8") as file:
        desc = desc.split()
        for word in desc:
            if not word in stop_words and not word in moreWords:
